...
(function (window, document, undefined) {
    ...
    var factory = function ($, DataTable) {
        ...
        // increase min column width to reduce likelihood user resize columns into a bad state
        var MIN_COL_WIDTH = 60; // NUTANIX CUSTOM - ENG-79302: was 10
        ...
        ColResize.prototype = {
             ...
             */
            "fnReset": function () {
                if (!this.s) { return this; } // NUTANIX CUSTOM - ENG-79302 - null check safe guard
                for (var i = 0, iLen = this.s.dt.aoColumns.length; i < iLen; i++) {
                    // ENG-79302 { update sWidth and sWidthOrig instead of width - see dataTables.js fnAdjustColumnSizing
                    // this.s.dt.aoColumns[i].width = this.s.dt.aoColumns[i]._ColResize_iOrigWidth;
                    this.s.dt.aoColumns[i].sWidth = this.s.dt.aoColumns[i]._ColResize_iOrigWidth;
                    this.s.dt.aoColumns[i].sWidthOrig = this.s.dt.aoColumns[i]._ColResize_iOrigWidth;
                    // ENG-79302 }
                }

                // NUTANIX BEGIN CUSTOM - ENG-79302 { adjust() does not exist in the version of dataTables we use
                // this.s.dt.adjust().draw();
                this.s.dt.oInstance.fnAdjustColumnSizing(true /* redraw */, true /* auto width */);
                // NUTANIX END CUSTOM - ENG-79302 }
            ...
            "_fnConstruct": function () {
                ...
                for (i = 0; i < iLen; i++) {
                    ...
                    // NUTANIX BEGIN CUSTOM - ENG-79302 { save sWidthOrig instead of width which tends to be undefined at this stage
                    // this.s.dt.aoColumns[i]._ColResize_iOrigWidth = this.s.dt.aoColumns[i].width;
                    this.s.dt.aoColumns[i]._ColResize_iOrigWidth = this.s.dt.aoColumns[i].sWidthOrig;
                    // NUTANIX END CUSTOM - ENG-79302 }
                }
            ...
            "_fnStateSave": function (oState) {
                oState.columns = oState.columns || []; // added by shyan @ nutanix
                this.s.dt.aoColumns.forEach(function (col, index) {
                    oState.columns[index] = oState.columns[index] || {}; // added by shyan @ nutanix
            ...
            "_fnMouseMove": function (e) {
                ...
                // NUTANIX BEGIN CUSTOM - ENG-79302 { special treatment for first column, which is usually a checkbox, so allow a smaller min width
                var is_first = ($(that.s.mouse.targetColumn.nTh).prev("th").length == 0);
                var _MIN_COL_WIDTH_ = is_first ? 15 : MIN_COL_WIDTH;

                //Get the minimum width of the column (default minimum 10px)
                var minColumnWidth = Math.max(parseInt($(that.s.mouse.targetColumn.nTh).css('min-width')), _MIN_COL_WIDTH_);
                // NUTANIX END CUSTOM - ENG-79302 }
                ...
                if ((dx > 0 && distFromRight <= 0) || (dx < 0 && distFromRight >= 0)) {
                    if (!that.s.init.tableWidthFixed) {
                        ...
                        if (that.s.mouse.neighbourColumn) {
                            ...
                            var minColumnNeighbourWidth = Math.max(parseInt($(that.s.mouse.neighbourColumn.nTh).css('min-width')), MIN_COL_WIDTH); // NUTANIX CUSTOM - ENG-79302
                            ...
                            if (dx > 0) {
                                // NUTANIX CUSTOM - ENG-79302: only do it if nearest neighbor has enough room to reduce
                                if ( - widthDiffNeighbour >= dx) { // ENG-79302
                                    domNeighbourCols.width(that.s.mouse.neighbourColumn.width);
                                    domCols.width(that.s.mouse.targetColumn.width);
                                }
                            } else {
                                // NUTANIX CUSTOM - ENG-79302: in this case make sure nearest neighbor column doesn't grow at other columns' expense
                                if (newColWidth !== minColumnWidth) { // NUTANIX CUSTOM - ENG-79302
            ...
            "_fnResizeHandleCheck": function (e, nTh) {
                ...
                if (resizeAvailable) {
                    // NUTANIX BEGIN CUSTOM
                    // $(nTh).css("cursor", "ew-resize");
                    $(nTh).addClass("col_resize");
                    // NUTANIX END CUSTOM
            ...
            "_fnDestroy": function () {
                ...
                if (that.s.init.destroyCallback) {
                    that.s.init.destroyCallback.call(that);
                }
