...
//
// UserAuditHistoryChartView used for user audit history chart.
//
define([
  // Core
  'views/graph/EntityBarChartView',
  // Models
  'models/usersaudithistory/UserAuditGraphModel',
  // Utils
  'utils/AppConstants',
  // Templates
...
function(
  ...
  var UserAuditHistoryChartView = EntityBarChartView.extend({
    ...
    initialize: function(options) {
      this.fetch = 0;
      this.model = new UserAuditGraphModel();
      const userId = options.userId;
      const startTimeInMs = options.startTimeInMs;
      const endTimeInMs = options.endTimeInMs;

      // Setting the URL parameters
      // Adding userId start time and end time to the URL
      this.setUrlParams(userId, startTimeInMs, endTimeInMs);
  ...
  return UserAuditHistoryChartView;
});
