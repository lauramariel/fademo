...
//
// UserMachineAuditHistoryTableView enables the user to view the table with
// the user machine audit history.
//
define([
  // Core
  ...
  'collections/usermachineaudithistory/UserMachineAuditHistoryCollection',
  'collections/usermachineaudithistory/UserMachineAuditHistoryExportCollection',
  // Models
...
function(
  ...
  UserMachineAuditHistoryCollection,
  UserMachineAuditHistoryExportCollection,
  // Models
  FileSearchModel,
  TaskModel,
  // Managers
  ...
  var UserMachineAuditHistoryTableView = BaseTableView.extend({
    ...
    initialize: function(options) {
      ...
      this.model = new UserMachineAuditHistoryCollection();
      this.parent = options.parent;
      this.setUrlParams(options.machineId);
      ...
      let retArray = [
        {
          'sTitle'  : 'User Name',
          'sWidth'  : '25%',
          'mData'   : UserMachineAuditHistoryModel.DP.AUDIT_USERNAME,
          'mRender' : function(data, type, full) {
            let userName = data || AppConstants.NOT_AVAILABLE;
            if (type === AppConstants.TABLE_COL_TYPE_EXPORT) {
              userName =
                full[UserMachineAuditHistoryModel.DP.AUDIT_USERNAME] ||
                AppConstants.NOT_AVAILABLE;
            }

            return '<span title="' + userName + '">' +
              userName + '</span>';
          }
        },
        {
          'sTitle'  : 'Operation',
          'sWidth'  : '25%',
          'mData'   : UserMachineAuditHistoryModel.DP.AUDIT_OPERATION,
          'mRender' : function(data, type, full) {
            ...
            if (type === AppConstants.TABLE_COL_TYPE_EXPORT) {
              const operation =
                full[UserMachineAuditHistoryModel.DP.AUDIT_OPERATION];
              auditOperation =
              ...
              if (_.indexOf(AppConstants.PERMISSION_DENIED_OP_STATUS,
                full[UserMachineAuditHistoryModel.DP.AUDIT_STATUS])
                > -1) {
                ...
                full[UserMachineAuditHistoryModel.DP.AUDIT_STATUS]) > -1) {
                const operationVal = auditOperation;
                auditOperation =
        ...
        },
        {
          ...
          'mRender' : function(data, type, full) {
            ...
            if (type === AppConstants.TABLE_COL_TYPE_EXPORT) {
              ...
              if ((full[UserMachineAuditHistoryModel.DP.AUDIT_OPERATION] ===
                AppConstants.OPERATION.Rename) &&
                (_.indexOf(AppConstants.FILE_BLOCKING_PD_OP_STATUS,
                  full[UserMachineAuditHistoryModel.DP.AUDIT_STATUS]) < 0)) {
                let oldFileName = oldFileTempl({
                  oldName: full.audit_old_objectname
                });

                retVal = full.audit_objectname + oldFileName;
              } else {
                retVal = full.audit_objectname ||
                  AppConstants.NOT_AVAILABLE;
              }
            } else {
              let auditPath = '';
              ...
              if ((full[UserMachineAuditHistoryModel.DP.AUDIT_OPERATION] ===
                ...
                let oldFile = full.audit_old_objectname ||
                  AppConstants.NOT_AVAILABLE,
                    oldFileArr = [], len = 0;

                if (full.audit_old_objectname.includes('/')) {
                  oldFileArr = full.audit_old_objectname.split('/');
                ...
                retVal = '<span class="inline-popover" actionTargetFile = '
                  + full.audit_object_ID + ' actionTargetOldParentId = ' +
                  full.audit_old_parent_ID + ' actionTargetOldName = "' +
                  full.audit_old_objectname + '" data-toggle="popover">' +
                  auditPath + '</span>';
              } else {
                retVal = '<span class="inline-popover" actionTargetFile = ' +
                  full.audit_object_ID + ' data-toggle="popover">' +
        ...
        },
        {
          ...
          'mData'   : 'audit_event_date',
          'mRender' : function(data, type, full) {
            let auditTimestamp = '';
            if (type === AppConstants.TABLE_COL_TYPE_EXPORT) {
              auditTimestamp =
                TimeUtil.formatDate(full.audit_event_date);
            } else {
              auditTimestamp = TimeUtil.formatDate(data);
            }
      ...
      this.fileModel.fetch({
        success: function(data) {
          if (data && data.attributes.path) {
            ...
            $(fileObj.popoverId).html('Path not available!');
          }
        },
        error: function(xhr) {
    ...
    initiateExport: function(e) {
      ...
      var exportCollection = new UserMachineAuditHistoryExportCollection();
      ...
      exportCollection.getURL(this.options.machineId, exportChunkCount,
        this.options.startTimeInMs, this.options.endTimeInMs);
      ...
      let taskModel = new TaskModel({
        ...
        message: 'Downloading (' + type + ') audit history for: ' +
          this.options.machineName.replace('%5C', '\\'),
        percent: 0 });
        ...
        success: function(data) {
          ...
          if (recursiveCallCount <= 1) {
            ...
            taskModel.setMessage('Downloaded (' + exportType + ') audit ' +
              'history for: ' + _this.options.machineName.replace('%5C', '\\'));
  ...
  return UserMachineAuditHistoryTableView;
});
