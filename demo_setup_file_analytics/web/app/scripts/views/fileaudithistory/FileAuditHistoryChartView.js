...
//
// FileAuditHistoryChartView used for file audit history chart.
//
define([
  // Core
...
function(
  ...
  var FileAuditHistoryChartView = EntityBarChartView.extend({

    model: null,

    // Operation filter has permission denied or not
    ...
    initialize: function(options) {
      this.fetch = 0;
      this.model = new FileAuditGraphModel();
      const fileId = options.userInput.replace(/\\/g, '%5C');
      const startTimeInMs = options.startTimeInMs;
      const endTimeInMs = options.endTimeInMs;
      let objectType = AppConstants.FILE_SEARCH_TYPE.FILE;

      // In case of directory audit history, set the type to directory.
      if (options.objectType === AppConstants.FILE_SEARCH_TYPE.DIRECTORY) {
        objectType = AppConstants.FILE_SEARCH_TYPE.DIRECTORY;
      }

      // Setting the URL parameters
      this.setUrlParams(fileId, startTimeInMs, endTimeInMs, objectType);
      EntityBarChartView.prototype.initialize.call(this, options);
    },

    // @private
    // Sets the parameter to create the URL.
    setUrlParams: function(searchVal, startTimeInMs, endTimeInMs, objectType) {
      // Getting the URL for the model
      this.model.getURL(searchVal, startTimeInMs, endTimeInMs, objectType);
    ...
    // Need to make 3 separate calls 1. All operations 2. Permission Denied
    // 3. Permission Denied File Blocking.
    fetchGraphData: function(filterDuration) {
      ...
      if (((this.hasPermissionDenied || this.hasPermissionDeniedFileBlocking) &&
        (operationFilter.operations.length === 1)) ||
        ((this.hasPermissionDenied && this.hasPermissionDeniedFileBlocking) &&
    ...
    prefetchPermissionDeniedStats: function(filterDuration) {
      ...
      // Check for permission denied access control or when no operation
      // is selected
      ...
      // Check for permission denied file blocking or when no operation
      // is selected
    ...
    // Function to return the applied operation filters
  ...
  // Return FileAuditHistoryChartView
  return FileAuditHistoryChartView;
});
